#!/usr/bin/env bash
set -ex

export CYPRESS_AUTH_PASSWORD="${AUTH_PASSWORD}"
export CYPRESS_AUTH_USERNAME="${AUTH_USERNAME}"
export CYPRESS_AUTH_DOMAIN="${AUTH_DOMAIN}"
export CYPRESS_AUTH_AUDIENCE="${AUTH_AUDIENCE}"
export CYPRESS_AUTH_CLIENT_ID="${AUTH_CLIENT_ID}"
export CYPRESS_AUTH_CLIENT_SECRET="${AUTH_CLIENT_SECRET}"
IS_RELEASE=${IS_RELEASE:-false}
VERSION=${VERSION:-"v0.0.0"}
VERSION=${VERSION:1}
WORKING_DIRECTORY=$(pwd)
CONFIGURATION=${CONFIGURATION:-'Release'}
PUBLISH_DIRECTORY="${WORKING_DIRECTORY}/publish"
TEST_RESULTS_DIRECTORY="${WORKING_DIRECTORY}/test-results"
COVERAGE_REPORT_GLOB="${TEST_RESULTS_DIRECTORY}/**/coverage.cobertura.xml"
COVERAGE_EXCLUDE_FILTERS="[Haus.Testing.*]*,[*]Haus.Core.Common.Storage.Migrations*"

SITE_HOST_PUBLISH_DIRECTORY="${PUBLISH_DIRECTORY}/haus-site"
SITE_HOST_BASE_ASSET_PATH="Haus.Site.Host"
SITE_HOST_PROJECT="${WORKING_DIRECTORY}/src/Haus.Site.Host/Haus.Site.Host.csproj"

WEB_HOST_PUBLISH_DIRECTORY="${PUBLISH_DIRECTORY}/haus-web"
WEB_HOST_BASE_ASSET_PATH="Haus.Web.Host"
WEB_HOST_PROJECT="${WORKING_DIRECTORY}/src/Haus.Web.Host/Haus.Web.Host.csproj"

ZIGBEE_HOST_PUBLISH_DIRECTORY="${PUBLISH_DIRECTORY}/haus-zigbee"
ZIGBEE_HOST_BASE_ASSET_PATH="Haus.Zigbee.Host"
ZIGBEE_HOST_PROJECT="${WORKING_DIRECTORY}/src/Haus.Zigbee.Host/Haus.Zigbee.Host.csproj"

ZIGBEE_SIMULATOR_PUBLISH_DIRECTORY="${PUBLISH_DIRECTORY}/haus-zigbee-simulator"
ZIGBEE_SIMULATOR_BASE_ASSET_PATH="Haus.Zigbee.Simulator"
ZIGBEE_SIMULATOR_PROJECT="${WORKING_DIRECTORY}/src/Haus.Zigbee.Simulator/Haus.Zigbee.Simulator.csproj"
