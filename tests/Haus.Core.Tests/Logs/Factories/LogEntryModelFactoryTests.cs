...
namespace Haus.Core.Tests.Logs.Factories
{
    public class LogEntryModelFactoryTests
    {
        ...
        public void WhenLogLineProvidedThenCreatesLogEntryFromLine()
        {
            ...
            entry.Message.Should().Be("RX (413 bytes) <<< \"Publish: [Topic=haus/commands] [Payload.Length=396] [QoSLevel=AtMostOnce] [Dup=False] [Retain=False] [PacketIdentifier=]\"");
        }
        ...
        public void WhenLogLineProvidedReturnsPropertiesFromLine()
        {
            ...
            jObject.Value<string>("@m").Should().Be("RX (413 bytes) <<< \"Publish: [Topic=haus/commands] [Payload.Length=396] [QoSLevel=AtMostOnce] [Dup=False] [Retain=False] [PacketIdentifier=]\"");
        }
