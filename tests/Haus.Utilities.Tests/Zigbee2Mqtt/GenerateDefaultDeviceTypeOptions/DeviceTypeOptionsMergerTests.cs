...
public class DeviceTypeOptionsMergerTests
{
    ...
    {
        var merged = _merger
            .Merge(Array.Empty<DeviceTypeOptions>(), new[] { new DeviceTypeOptions("vendor", "model") })
            .ToArray();
    ...
    public void WhenExistingOptionsContainsDeviceTypeOptionThenReturnsExistingInMerged()
    {
        var merged = _merger
            .Merge(
                new[] { new DeviceTypeOptions("vendor", "model", DeviceType.LightSensor) },
                new[] { new DeviceTypeOptions("vendor", "model") }
            )
            .ToArray();
    ...
    {
        var merged = _merger
            .Merge(new[] { new DeviceTypeOptions("here", "one") }, new[] { new DeviceTypeOptions("other", "two") })
            .ToArray();
    ...
    {
        var merged = _merger
            .Merge(new[] { new DeviceTypeOptions("Philips", "idk") }, new[] { new DeviceTypeOptions("ABC", "three") })
            .ToArray();
